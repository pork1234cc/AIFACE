...
def default_params() -> dict:
    return {
        "hair_source_asset_id": None,
        "glasses_keep": True,
        "clothes_mode": "simplified",
        "clothes_source_asset_id": None,
        "background": "white",
        "aspect_ratio": "1:1",
        "extra_requirement": "",
    }
...
class Asset(Base):
    ...
    __table_args__ = (
        CheckConstraint("kind IN ('input','generated')", name="ck_assets_kind"),
        CheckConstraint(
        ...
        Index("ix_assets_order_id", "order_id"),
        Index(
            "uq_assets_active_role",
            "order_id",
            "input_role",
            unique=True,
            sqlite_where=text("is_active_input=1 AND input_role IN ('person_main','reference')"),
        ),
    )
    ...
    generation_task_id: Mapped[str | None] = mapped_column(
        ForeignKey("generation_tasks.id", name="fk_assets_generation_task"), unique=True
    )
...
class GenerationBatch(Base):
    ...
    __table_args__ = (
        UniqueConstraint("order_id", "operation", "request_key", name="uq_batch_request"),
        CheckConstraint(
            "(operation='initial' AND target_count IN (1,2)) OR "
            "(operation='revision' AND target_count=1)",
            name="ck_batch_target",
        ),
        Index("ix_batches_order_created", "order_id", "created_at"),
        Index(
            "uq_batch_open_order",
            "order_id",
            unique=True,
    ...
    base_asset_id: Mapped[str | None] = mapped_column(ForeignKey("assets.id", use_alter=True))
...
class GenerationTask(Base):
    ...
    __table_args__ = (
        UniqueConstraint("batch_id", "slot_index", "attempt_no", name="uq_task_attempt"),
        CheckConstraint("slot_index IN (0,1) AND attempt_no>=1", name="ck_task_slot_attempt"),
        Index("ix_tasks_poll", "status", "next_poll_at"),
    )

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id)
    batch_id: Mapped[str] = mapped_column(ForeignKey("generation_batches.id"))
...
class GenerationAction(Base):
    ...
    __table_args__ = (UniqueConstraint("scope", "request_key", name="uq_action_request"),)
...
class OrderDelivery(Base):
    ...
    __table_args__ = (
        Index("ix_deliveries_order", "order_id"),
        Index(
            "uq_delivery_active",
            "order_id",
            unique=True,
        ...
        CheckConstraint("sort_index >= 0", name="ck_delivery_sort"),
    )
